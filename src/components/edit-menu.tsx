...
} from '../utils/detectObject.ts';
import {
  loadBgRemovalModel,
  BgRemovalSession,
} from '../utils/removeBackground.ts';
...
export default function EditMenu({ theme }: { theme: boolean }) {
  ...
  const [objectDetectionSession, setObjectDetectionSession] =
    useState<sessionConfig | null>(null);
  const [backgroundRemovalSession, setBackgroundRemovalSession] =
    useState<BgRemovalSession | null>(null);
  ...
  useEffect(() => {
    async function initializeModel() {
      cv['onRuntimeInitialized'] = async () => {
        try {
          const { model, nms } = await loadObjectDetectionModel(
          ...
          throw new Error(`Error occured during initialize model: ${error}`);
        }
      };
    }
    initializeModel();
  ...
  return (
    <div
      className={`${theme ? 'border-white bg-black' : 'border-black bg-white'} flex justify-between items-center gap-12 p-12 rounded-md border-solid border-2 border-b-8 border-r-8 border-black max-w-[80vw] lg:max-w-screen lg:min-h-[50vh]`}
    >
