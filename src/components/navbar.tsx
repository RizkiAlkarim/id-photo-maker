  ...
  return (
    <div className="min-h-[10%] px-12 py-5 border-solid border-b-2">
      <div className="flex justify-between items-center">
        <a href="/"><img src={Logo} className={`${theme ? "invert" : "invert-0"}`}/></a>
        <img src={theme ? lightIcon : darkIcon} className={`${theme ? "invert" : "invert-0"} transition cursor-pointer`} onClick={() => handleTheme(!theme)}/>
      </div>
    </div>
  )
}
