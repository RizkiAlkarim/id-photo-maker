...
export async function loadObjectDetectionModel(
  config: config
): Promise<objectDetectionSession> {
  env.wasm.wasmPaths = './';
  ...
  const { output_selected } = await nms.run({
    ...
    iou_threshold: new Tensor(
      'float32',
      new Float32Array([config.iouThreshold])
    ),
    score_threshold: new Tensor(
      'float32',
      new Float32Array([config.scoreThreshold])
    ),
  });
...
export async function runObjectDetection(
  ...
  const [preprocessedSrc, xRatio, yRatio] = preprocess(
    src,
    config.inputShape[2],
    config.inputShape[3]
  );

  const inputTensor = new Tensor('float32', preprocessedSrc.data32F, [
    1,
    3,
    config.inputShape[3],
    config.inputShape[2],
  ...
  for (let i = 0; i < output_selected.dims[1]; i++) {
    ...
    const rawData = output_selected.data.slice(
      startIndex,
      endIndex
    ) as Float32Array;
  ...
  if (results && results.length > 0) {
    const [x, y, width, height] = results[0].bbox.map((b: number) =>
      Number(Math.round(b))
    );

    const yCoordinateToleration = 10;
    let adjustedY = y;
    if (y < yCoordinateToleration) adjustedY = 0;
    else adjustedY -= yCoordinateToleration;

    const scaledHeight = width;
    let croppedHeight = height;
    if (height >= scaledHeight) croppedHeight = scaledHeight;

    const cropRect = new cv.Rect(x, adjustedY, width, croppedHeight);
